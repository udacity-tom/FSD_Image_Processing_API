...
const logger = (req: {query: any; path: any;}, res: any, next:() => void ) => {
    ...
    console.log(`${getTimeAndDate()} ${req.path} visited.`+
        (req.path != '/convertImage' ? ' ' : ` Filename ${req.query.filename} will be resized to width:${req.query.width} and height:${req.query.height}`)+
